...
- Используй web_search только когда пользователь явно просит найти что-то в интернете
- Например: "найди новости по BTC", "что случилось с AAVE", "поищи эксплойт KelpDAO"
- Для вопросов о монетах из базы — сначала ищи в базе через tools, и только если пользователь отдельно просит новости — используй web_search
...
async def claude_reply(user_id: int, user_text: str) -> str:
    ...
    try:
        # ── Первый запрос с инструментами + web search ──
        ...
        max_tool_calls = 5  # увеличено с 3 до 5 — web search тоже использует слоты
        while response.stop_reason == "tool_use" and tool_calls_count < max_tool_calls:
            ...
            for block in response.content:
                if block.type == "tool_use":
                    # web_search обрабатывается самим API, нам нужно передать результат дальше
