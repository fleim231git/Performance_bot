...
- Для монет трейдера используй простой список:
  1. #STOUSDT — +330$ | ⚡️10.50% (8.00%–14.00%) | tp=2.1%
  2. #SIRENUSDT — +306$ | ⚡️4.50% (0.65%–7.00%) | tp=1.8%
- Показывай: профит, дистанс с диапазоном мин–макс, тейк профит
- ⚡️ = рабочий дистанс (разброс >3%), 🎯 = стабильный дистанс (разброс ≤3%), 🛡 = страховочный
- НЕ показывай WR и буфер в списке монет — только если прямо спрашивают
- Анализ пиши кратко — 2-3 строки, без лишних заголовков
...
def execute_tool(tool_name: str, tool_input: dict) -> str:
    ...
    try:
        if tool_name == "get_trader_stats":
            ...
            real_count = len(top_coins)
            result += f"Показано монет: {real_count} — используй ИМЕННО это число в ответе, не пиши больше\n"
            for coin, p, n, dmin, dmax, tp, wins in top_coins:
                dist_data = get_smart_distance(conn, where, params, coin)
                result += f"#{coin}: {'+' if p>=0 else ''}{(p or 0):.2f}$"
                if dmin and dmax:
                    if dist_data and dist_data.get("smart"):
                        result += f" | ⚡️{fmt_dist(dist_data['working'])} 🛡{fmt_dist(dist_data['insurance'])} ({fmt_dist(dmin)}–{fmt_dist(dmax)})"
                    else:
                        result += f" | 🎯{fmt_dist(dmin)}–{fmt_dist(dmax)}"
            ...
            result += f"Показано монет: {len(rows)}{dist_filter} — пиши ИМЕННО это число в заголовке\n\n"
