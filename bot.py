...
def get_db_context() -> str:
    ...
    ctx += "\n=== ТОП-5 ТРЕЙДЕРОВ СЕГОДНЯ ===\n"
    for t, cnt, p, w in (traders_today or []):
...
- Tool возвращает [СЧЁТЧИК:N] — используй ТОЛЬКО это число в заголовке
- [СЧЁТЧИК:9] → пиши "9 монет". Никогда не пиши больше чем в данных
- [СЧЁТЧИК:N] — служебная метка, НЕ показывай её пользователю
- Tool разделяет монеты на 🟢 прибыльные и 🔴 убыточные — сохраняй эту структуру
...
━━━ ПРАВИЛО 5 — РЕЙТИНГ ━━━
- Низкий дистанс ≠ лучшая монета (тейк тоже меньше)
- Лучшая монета = profit_usd × win_rate (эффективность)
- Не хвали монету только за низкий дистанс если профит маленький
...
TOOLS = [
    ...
    {
        "name": "get_top_coins",
        "description": "Получить топ монет с дистансами за период. Можно фильтровать по диапазону дистанса и бирже.",
        "input_schema": {
            "type": "object",
            "properties": {
                "since": {"type": "string", "description": "Дата/время начала. Форматы: YYYY-MM-DD или YYYY-MM-DDTHH:MM:SS для запросов по часам/минутам. Примеры: 2026-04-01 или 2026-04-01T18:00:00"},
...
def execute_tool(tool_name: str, tool_input: dict) -> str:
    ...
    try:
        if tool_name == "get_trader_stats":
            ...
            result += f"[СЧЁТЧИК:{len(top_coins)}] монет ({sort_label}):\n"
            ...
            result = f"[СЧЁТЧИК:{len(rows)}]{dist_filter}\n"
            if profit_rows:
...
async def cmd_coins(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dists:
        r += "\n📐 *Дистансы (топ монеты):*\n"
        conn2 = sqlite3.connect(DB_PATH)
        w2 = where  # уже содержит фильтр биржи
        p2 = list(params)
