...
def execute_tool(tool_name: str, tool_input: dict) -> str:
    ...
    try:
        if tool_name == "get_trader_stats":
            ...
            result = f"[ДАННЫЕ ДЛЯ ОТВЕТА — ИСПОЛЬЗУЙ ТОЧНО ЭТИ ЦИФРЫ И ДИСТАНСЫ]\n"
            result += f"Всего монет: {len(rows)}{dist_filter}\n\n"

            if profit_rows:
                result += f"ПРИБЫЛЬНЫЕ ({len(profit_rows)}):\n"
                for i, row in profit_rows:
                    result += f"{i}. {fmt_coin_row(*row)}\n"

            if loss_rows:
                result += f"\nУБЫТОЧНЫЕ ({len(loss_rows)}):\n"
                for i, row in loss_rows:
                    result += f"{i}. {fmt_coin_row(*row)}\n"
