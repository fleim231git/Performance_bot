...
- Если инструмент вернул 5 монет — пиши "Топ-5", не "Топ-15" и не "Топ-20"
- НИКОГДА не пиши в заголовке число больше реального количества строк в данных
- Считай строки перед написанием заголовка
...
def execute_tool(tool_name: str, tool_input: dict) -> str:
    ...
    try:
        if tool_name == "get_trader_stats":
            ...
            where = "WHERE distance IS NOT NULL AND distance > 0"
            params = []
            if since:
                where += " AND timestamp>=?"
                params.append(parse_dt(since))
            ...
            result = f"Топ {len(rows)} монет{dist_filter}:\n"
