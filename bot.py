...
def get_stats_for_period(since: str, until: str = None) -> str:
    ...
    r += "🏆 *Топ-5 монет:*\n"
    for i, (coin, p, cnt, dmin, dmax, tp) in enumerate(top_coins, 1):
        dist_data = get_smart_distance(conn, where, list(params), coin)
        dist_str = fmt_dist_info(dist_data) if dist_data else ""
        tp_str = f" | tp={tp:.1f}%" if tp else ""
        r += f"{i}. #{coin} {'+' if p>=0 else ''}{p:.2f}$ ({cnt} сделок){dist_str}{tp_str}\n"
...
def get_smart_distance(conn, where: str, params: list, coin: str) -> dict | None:
    ...
    # 90-й перцентиль — всегда >= рабочего
    idx = int(len(all_dists) * 0.9)
    insurance = all_dists[min(idx, len(all_dists) - 1)]
    # Страховочный не может быть меньше рабочего
...
def execute_tool(tool_name: str, tool_input: dict) -> str:
    ...
    try:
        if tool_name == "get_trader_stats":
            ...
            # Анализ дистансов + разбивка по биржам
            all_dist_data = []
            ...
            # Дистансы по биржам для каждой монеты
            EXCHANGES = ["Binance", "Bybit", "OKX"]
            result += "\nДИСТАНСЫ ПО БИРЖАМ:\n"
            for coin, pnl, cnt, dist, dmin, dmax, buf, wins in rows[:5]:  # топ-5
                result += f"#{coin}:\n"
...
async def handle_dialog_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Голосовое — реагируем только если reply боту или упоминание
    ...
    # Получаем thread_id если группа с темами
    thread_id = msg.message_thread_id if msg.is_topic_message else None

    await msg.chat.send_action("typing", message_thread_id=thread_id)
    reply = await claude_reply(user_id, clean_text)

    # Разбиваем длинные ответы на части по 4000 символов
    max_len = 4000
...
async def cmd_trader(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute(f'''SELECT coin, AVG(distance), AVG(buffer), AVG(take_profit), COUNT(*), SUM(profit_usd)
               ...
               GROUP BY coin ORDER BY SUM(profit_usd) DESC LIMIT 7''', params)
