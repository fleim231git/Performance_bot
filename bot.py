...
СТРОГОЕ ПРАВИЛО 3 — ТОЧНОЕ КОЛИЧЕСТВО:
- Tool возвращает строку "Всего монет в выборке: N" — используй ИМЕННО это число N
- Если tool вернул 9 строк монет — пиши "9 монет", не "13" и не "15"
- НИКОГДА не пиши число в заголовке которого нет в данных от tool
- Отдельно считай прибыльные и убыточные — tool уже разделяет их на 🟢 и 🔴

СТРОГОЕ ПРАВИЛО 4 — ДИСТАНС ЭТО ПРОЦЕНТЫ:
- Дистанс и буфер — это проценты от цены. Всегда пиши со знаком %
- Правильно: "дистанс 1.93%" или "0.77%", НЕ просто "1.93" или "0.77"
- Рабочий дистанс (⚡️) — медиана дистансов прибыльных сделок
- Страховочный дистанс (🛡) — 90-й перцентиль всех дистансов
- Показывай рабочий/страховочный только если разброс > 3%

СТРОГОЕ ПРАВИЛО 5 — РЕЙТИНГ МОНЕТ:
- Низкий дистанс НЕ означает что монета лучше — тейк-профит тоже меньше
- Лучшая монета = высокий profit_usd × win_rate (эффективность)
- Не называй монеты "лучшими по дистансу" если у них маленький профит

СТРОГОЕ ПРАВИЛО 6 — ЗАПРОСЫ ПО ЧАСАМ:
- В базе timestamp хранится с точностью до секунды (формат: 2026-04-01T18:32:45)
- Ты МОЖЕШЬ делать запросы по часам и минутам — используй get_period_stats с since в формате YYYY-MM-DDTHH:MM:SS
- "за последний час" → since = текущее_время - 1 час
- "с 18:00" → since = сегодня T18:00:00
- "за 2 часа" → since = текущее_время - 2 часа
- НИКОГДА не говори что нет доступа к данным по часам — они ЕСТЬ
...
- Статистику за сегодня, неделю, любой месяц, любой час
...
def fmt_dist_info(dist_data: dict | None) -> str:
    ...
    if dist_data.get("smart"):
        ...
        return f" dist={fmt_dist(dist_data['avg'])} ({fmt_dist(dist_data['min'])}–{fmt_dist(dist_data['max'])})"
...
TOOLS = [
    ...
    {
        "name": "get_top_coins",
        "description": "Получить топ монет с дистансами за период. Можно фильтровать по диапазону дистанса.",
        "input_schema": {
            "type": "object",
            "properties": {
                "since": {"type": "string", "description": "Дата/время начала. Форматы: YYYY-MM-DD или YYYY-MM-DDTHH:MM:SS для запросов по часам/минутам. Примеры: 2026-04-01 или 2026-04-01T18:00:00"},
        ...
        if tool_name == "get_trader_stats":
            ...
            for coin, p, n, dist, dmin, dmax, buf, tp in top_coins:
                ...
                if dist and dmin and dmax:
                    ...
                    result += fmt_dist_info(dist_data)
                if buf: result += f" buf={fmt_dist(buf)}"
            ...
            profit_rows = [r for r in rows if (r[1] or 0) >= 0]
            loss_rows   = [r for r in rows if (r[1] or 0) < 0]

            def fmt_coin_row(coin, pnl, cnt, dist, dmin, dmax, buf, wins):
                wr = round((wins or 0)/cnt*100 if cnt > 0 else 0, 1)
                line = f"#{coin}: {'+' if pnl>=0 else ''}{(pnl or 0):.2f}$ WR={wr}%"
                if dist:
            ...
            result = f"Всего монет в выборке: {len(rows)}{dist_filter}\n"
            if profit_rows:
                result += f"\n🟢 Прибыльные ({len(profit_rows)}):\n"
                for row in profit_rows:
                    result += fmt_coin_row(*row) + "\n"
            if loss_rows:
                result += f"\n🔴 Убыточные ({len(loss_rows)}):\n"
...
async def cmd_coins(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dists:
        ...
        for coin, dist, buf, tp, cnt, p in dists:
            dist_data = get_smart_distance(conn2, w2, p2, coin)
            t = f"{tp:.1f}%" if tp else "н/д"
            r += f"  #{coin}:{fmt_dist_info(dist_data)} buf={fmt_dist(buf)} tp={t} | {'+' if p>=0 else ''}{p:.1f}$\n"
        conn2.close()
...
def build_daily_report() -> str:
    ...
    # Топ трейдеров за сегодня
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""SELECT trader, COUNT(*), SUM(profit_usd),
               SUM(CASE WHEN is_profit=1 THEN 1 ELSE 0 END)
               FROM trades WHERE timestamp>?
               GROUP BY trader ORDER BY SUM(profit_usd) DESC LIMIT 5""", (today,))
    traders_today = c.fetchall()
    if traders_today:
        r += "\n👤 *Топ трейдеров сегодня:*\n"
        for t, cnt, p, w in traders_today:
            wr = round((w or 0)/cnt*100 if cnt > 0 else 0, 1)
            r += f"  {'🟢' if p>=0 else '🔴'} *{t}*: {'+' if p>=0 else ''}{(p or 0):.2f}$ | {cnt} сделок | WR {wr}%\n"

    r += f"\n{'─'*25}\n📅 *НЕДЕЛЯ*\n"
    r += get_stats_for_period(week_ago)

    # Топ трейдеров за неделю
    c.execute("""SELECT trader, COUNT(*), SUM(profit_usd),
               SUM(CASE WHEN is_profit=1 THEN 1 ELSE 0 END)
               FROM trades WHERE timestamp>?
               GROUP BY trader ORDER BY SUM(profit_usd) DESC LIMIT 5""", (week_ago,))
    traders_week = c.fetchall()
    if traders_week:
        r += "\n👤 *Топ трейдеров за неделю:*\n"
        for t, cnt, p, w in traders_week:
            wr = round((w or 0)/cnt*100 if cnt > 0 else 0, 1)
            r += f"  {'🟢' if p>=0 else '🔴'} *{t}*: {'+' if p>=0 else ''}{(p or 0):.2f}$ | {cnt} сделок | WR {wr}%\n"
...
async def cmd_coin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if avg_d:
        ...
        conn2.close()
        if dist_data and dist_data.get("smart"):
            r += f"📐 Дистанс: ⚡️рабочий {fmt_dist(dist_data['working'])} | 🛡страховочный {fmt_dist(dist_data['insurance'])} | диапазон {fmt_dist(min_d)}–{fmt_dist(max_d)}\n"
        else:
            r += f"📐 Дистанс: avg={fmt_dist(avg_d)} min={fmt_dist(min_d)} max={fmt_dist(max_d)}\n"
    if avg_b:
