...
def execute_tool(tool_name: str, tool_input: dict) -> str:
    ...
    try:
        if tool_name == "get_trader_stats":
            ...
            result += f"Монет в списке: {len(top_coins)} ({sort_label}):\n"
            ...
            result = f"Монет в выборке: {len(rows)}{dist_filter}. В заголовке пиши ИМЕННО это число. Это полный список — НЕ добавляй монеты которых здесь нет.\n\n"
            for i, row in enumerate(rows, 1):
                result += f"{i}. {fmt_coin_row(*row)}\n"
