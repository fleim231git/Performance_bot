...
TOOLS = [
    {
        ...
        "description": "Получить статистику трейдера по монетам с дистансами за период",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    try:
        if tool_name == "get_trader_stats":
            ...
            c.execute(f"""SELECT coin, SUM(profit_usd), COUNT(*),
                         AVG(distance), MIN(distance), MAX(distance), AVG(buffer), AVG(take_profit)
                         FROM trades {where}
                         GROUP BY coin ORDER BY SUM(profit_usd) DESC LIMIT 10""", params)
            top_coins = c.fetchall()

            c.execute(f"""SELECT exchange, COUNT(*) FROM trades {where}
                         GROUP BY exchange ORDER BY COUNT(*) DESC""", params)
            exchanges = c.fetchall()

            result = f"Трейдер: {trader}\n"
            result += f"Сделок: {cnt} | PnL: {'+' if pnl>=0 else ''}{(pnl or 0):.2f}$ | WR: {wr}%\n"
            result += f"Биржи: {', '.join([f'{e}({n})' for e,n in exchanges if e])}\n\n"
            result += "Топ монет:\n"
            for coin, p, n, dist, dmin, dmax, buf, tp in top_coins:
                ...
                if dist:
                    ...
                    result += f" dist={dist:.2f}({dmin_s}-{dmax:.2f if dmax else 0:.2f})"
            ...
            if dist:
                dmin_s = f"{dmin:.2f}" if dmin and dmin > 0 else "н/д"
                result += f"Дистанс: avg={dist:.2f} min={dmin_s} max={dmax:.2f if dmax else 0:.2f}\n"
            ...
            for coin, pnl, cnt, dist, dmin, dmax, buf, wins in rows:
                ...
                if dist: result += f" dist={dist:.2f}({dmin_s}-{dmax:.2f if dmax else 0:.2f})"
                if buf: result += f" buf={buf:.2f}"
                result += f" ({cnt} сделок)\n"
