    ...
    try:
        if tool_name == "get_trader_stats":
            ...
            for coin, p, n, dmin, dmax, tp, wins in top_coins:
                profit_pct = round((wins or 0)/n*100 if n > 0 else 0, 1)
                # Рабочий дистанс = медиана прибыльных сделок
                ...
                if dmin and dmax:
                    if dist_data and dist_data.get("smart"):
                        result += f" ⚡️{fmt_dist(dist_data['working'])} ({fmt_dist(dmin)}–{fmt_dist(dmax)})"
                    else:
                        result += f" dist={fmt_dist(dmin)}–{fmt_dist(dmax)}"
